...
class Api {
  ...
  _load(url, options = {method: HttpMethod.GET}) {
    return this._http
      .request({url, ...options})
      .then(Api.getData)
  ...
  getHotArticles({limit}) {
    return this._load(
        `${ApiPath.ARTICLES}${ArticlesApiPath.POPULAR}`,
        {
          params: {
            limit
          }
  ...
  getArticleByCategoryId({id, limit, offset}) {
    return this._load(
        `${ApiPath.ARTICLES}${ArticlesApiPath.CATEGORIES}/${id}`,
        {
          params: {
            limit,
            offset
  ...
  getCategories(hasCount) {
    ...
    return this._load(ApiPath.CATEGORIES, {params});
  }
  ...
  getComments({limit, order, hasArticle}) {
    ...
    return this._load(
        `${ApiPath.ARTICLES}${ArticlesApiPath.COMMENTS}`,
        {
          params
        }
    );
  ...
  createComment(articleId, payload) {
    return this._load(
        `${ApiPath.ARTICLES}/${articleId}/comments`,
        {
          method: HttpMethod.POST,
          data: payload
        }
  ...
  loginUser(payload) {
    return this._load(
        `${ApiPath.USERS}${UsersApiPath.LOGIN}`,
        {
          method: HttpMethod.POST,
          data: payload
        }
